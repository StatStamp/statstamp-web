...
import { useMyTeams } from '@/hooks/teams';
...
export function TeamsContent() {
  ...
  const { data: teams, isLoading } = useMyTeams(debouncedSearch, !authLoading && !!user);
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">My Teams</h1>
            <Link
              href="/teams/new"
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              {debouncedSearch ? (
                ...
                <>
                  <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-1">No teams yet</p>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">Create your first team to get started.</p>
                  <Link
                    href="/teams/new"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">League</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Abbr</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Color</th>
                  ...
                  {teams.map((t) => (
                    ...
                    >
                      <td className="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                        <Link href={`/teams/${t.id}`} onClick={(e) => e.stopPropagation()} className="hover:underline">
                          {t.name}
                        </Link>
                      </td>
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">
                        {t.league_name ?? <span className="text-zinc-400 dark:text-zinc-600">—</span>}
                      </td>
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">
                        {t.abbreviation ?? <span className="text-zinc-400 dark:text-zinc-600">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${t.is_public ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400'}`}>
