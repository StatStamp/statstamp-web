...
import { useMyPlayers } from '@/hooks/players';
...
export function PlayersContent() {
  ...
  const { data: players, isLoading } = useMyPlayers(debouncedSearch, !authLoading && !!user);
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">My Players</h1>
            <Link
              href="/players/new"
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              {debouncedSearch ? (
                ...
                <>
                  <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-1">No players yet</p>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">Create your first player to get started.</p>
                  <Link
                    href="/players/new"
                  ...
                  {players.map((p) => (
                    ...
                    >
                      <td className="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                        <Link href={`/players/${p.id}`} onClick={(e) => e.stopPropagation()} className="hover:underline">
                          {p.name}
                        </Link>
                      </td>
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">
                        {p.number ?? <span className="text-zinc-400 dark:text-zinc-600">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${p.is_public ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400'}`}>
